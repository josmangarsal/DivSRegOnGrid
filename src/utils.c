...
REAL YInWindow(PREAL pX, INT WWidth) {
	...
	Yw = (REAL) HalfWWidth + (HalfWWidth) - Yw - (REAL) ((WWidth - (INT) EqualTRiangleHigh) / 2.0);

	return Yw;
}

/*---------------------------------------------------------------------------*/
VOID DrawCircle(PREAL pX, REAL Radius, INT NCircle, INT WWidth, PCHAR color, PCHAR dash) {
	REAL WRadius, Wx, Wy;
...
/*---------------------------------------------------------------------------*/
VOID DrawEpsCircle(PREAL pX, REAL Radius, INT NCircle, INT WWidth, PCHAR OutLineColor, PCHAR FillColor) {
	REAL WRadius, Wx, Wy;
...
/*---------------------------------------------------------------------------*/
/*Return the fraction point between pA and pB*/
/*If Fraction=0.5, it return the middle point*/
void FractionPoint(PREAL pA, PREAL pB, PREAL pNew, REAL Fraction, INT NDim) {
	int i;

	for (i = 0; i < NDim; i++)
		pNew[i] = pA[i] + (pB[i] - pA[i]) * Fraction;
}
