...
PCDSIMPLEX TwoUSC(PCDSIMPLEX pCDS_O, PPREAL ppVCoor_O, PPREAL ppVCoor_T, PREAL pCentreT, PPREAL ppCDSToVMat, PULINT pCountersCDS, REAL RedFact,
	...
	for (j = 0; j < NDim; j++) { //Number of new simplices
		...
		if (IsCovered) //Previous simplex was covered. Let's use its memory.
			...
			pCDS_T = NewCDSimplex(NDim, pCentreT, RedFact * pCDS_O->L, RedFact * pCDS_O->R, Up, True, NSimplex, DivPhase, pCDS_O->Level + 1);
...
void NextIF(INT CurrNGrid, REAL IncFact, PINT pNewNGrid, PREAL pNewIncFact) {
	/*Requires:
