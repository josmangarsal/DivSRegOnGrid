...
PLCDSNODE NewLCDSNode(PCDSIMPLEX pCDS)
{
 ...
 plcdsn        = (PLCDSNODE) GetMem ((SIZE)1,
                                     (SIZE)sizeof(LCDSNODE),"NewLCDSNode");
 plcdsn->pCDS  = pCDS;
 plcdsn->pnext = NULL;
...
/*---------------------------------------------------------------------------*/
VOID PrintListCDS (PLISTCDS plcds, INT NDim, PPREAL ppVCoorT, 
                   PPREAL ppCDSToVMat)
{
        ...
        if (pCDS->Up == pCDSaux->Up) //Only if they has the same orientation
           {
            ...
            for (j=0;j<NDim;j++)
                if ( (Up==True && 
                      LT( pCDS->pCentre[j]-pCDSaux->pCentre[j] + 
                      (pCDSaux->R - pCDS->R) / (REAL) NDim, 0.0) 
                     ) ||
                     (Up==False && 
                      LT( pCDSaux->pCentre[j] - pCDS->pCentre[j] + 
                      (pCDSaux->R - pCDS->R) / (REAL) NDim, 0.0)
                     )                     
                   )  
                   {
